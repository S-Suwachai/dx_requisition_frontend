...
const AppSidebar: React.FC<AppSidebarProps> = ({ navItems }) => {
  ...
  return (
    ...
    >
      <div className={`flex  ${!isExpanded && !isHovered ? "lg:justify-center py-8" : "justify-start py-2"}`}>
        <Link href="/">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <Image src="/images/logo/dx_bearing_logo.svg" alt="Logo" width={100} height={40} />
            </>
          ) : (
            <Image src="/images/logo/dx_bearing_logo.svg" alt="Logo" width={32} height={32} />
          )}
        </Link>
      </div>
